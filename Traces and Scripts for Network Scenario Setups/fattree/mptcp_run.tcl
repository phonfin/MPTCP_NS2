...
proc getPid { nid } {
	...
	global pod;
	set aggregator [expr $nid/($pod/2*$pod/2)];
	return $aggregator;
}
...
proc getSid { nid } {
	...
	global pod;
	set switch [expr ($nid%($pod/2*$pod/2))/($pod/2)];
	return $switch;
}
...
for {set i 0} {$i < $FLOW_NUM} {incr i} {
	...
	set ss [getSid $snid];    # the ToR where a sender belongs to
	set sh [getHid $snid];    # the sender's index in its ToR

	set rp [getPid $rnid];    # the pod where a receiver belongs to
	set rs [getSid $rnid];    # the ToR where a receiver belongs to
	set rh [getHid $rnid];    # the receiver's index in its ToR
	...
	for {set j 0} {$j<$NUM_SUB} {incr j} {
		...
		$ns attach-agent $hosts($sp,$ss,$sh) $tcp($fl);
		$ns attach-agent $hosts($rp,$rs,$rh) $tcp_sink($fl);
